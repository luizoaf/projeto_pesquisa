...
retorna_cluster = function(dados,k){
  ...
  dados$risco_b = ""
  dados$risco_b[dados$cluster == grupos[1]] = "conservador"
...
write.table(novos_pontos_classificados,file="agrupamento_distancias_volatilidade_mais_pontos.csv",row.names=F)
write.table(centroides,file="centroides_volatilidade.csv",row.names=F)
